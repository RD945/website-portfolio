...
for (const profile of [
  ...
  test(`cold-load performance: ${profile.name}`, async ({ browser }) => {
    ...
    for (let run = 0; run < 3; run++) {
      const context = await browser.newContext({ viewport: { width: profile.width, height: profile.height }, deviceScaleFactor: profile.width < 700 ? 2 : 1 });
      ...
      await session.send('Network.emulateNetworkConditions', { offline: false, latency: profile.latency, downloadThroughput: profile.mbps * 125000, uploadThroughput: 125000 });
      ...
      results.push(await page.evaluate(() => ({
        lcp: Math.round(window.metrics.lcp), cls: Number(window.metrics.cls.toFixed(4)),
        load: Math.round(performance.getEntriesByType('navigation')[0].loadEventEnd),
        transferredKB: Math.round(performance.getEntriesByType('resource').reduce((sum, r) => sum + r.transferSize, 0) / 1024),
        external: performance.getEntriesByType('resource').filter(r => !r.name.startsWith(location.origin)).length,
      })));
